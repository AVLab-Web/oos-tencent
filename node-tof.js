...
var _APPKEY = ''; // apply at http://tof.oa.com/, enable IPs of your machine
...
exports.mail = function(to, title, content, opts, cb) {
    if (content) {
        ...
        iSOAP('ws.tof.oa.com', '/MessageService.svc', 'http://tempuri.org/IMessageService/SendMail', data, function(res) {
            if (res) {
...
exports.passport = function(ticket, cb) {
    if (ticket) {
        ...
        iSOAP('login.oa.com', '/services/passportservice.asmx', 'http://indigo.oa.com/services/DecryptTicket', data, function(res) {
            if (res) {
                // console.log(res);
                var info = {},
...
exports.staff = function(uid, cb) {
    ...
    iSOAP('ws.tof.oa.com', '/StaffService.svc', 'http://tempuri.org/IStaffService/GetStaffInfoByName', data, function(res) {
        cb(res);
    });
    data = null;
};
...
exports.rtx = function(sender, receiver, content, title, cb) {
    ...
    data = data.replace(/>\{\w+\}</g, '><'); // trim parameters left
    iSOAP('ws.tof.oa.com', '/MessageService.svc', 'http://tempuri.org/IMessageService/SendRTX', data, function(res) {
        cb(res);
    });
    data = null;
};
...
exports.sms = function(sender, receiver, content, cb) {
    ...
    data = data.replace('>{msginfo}<', '>' + encode(content) + '<');
    data = data.replace(/>\{\w+\}</g, '><'); // trim parameters left
    iSOAP('ws.tof.oa.com', '/MessageService.svc', 'http://tempuri.org/IMessageService/SendSMS', data, function(res) {
        cb(res);
    });
    data = null;
};
...
exports.weixin = function(sender, receiver, content, cb) {
    ...
    data = data.replace('>{msginfo}<', '>' + encode(content) + '<');
    data = data.replace(/>\{\w+\}</g, '><'); // trim parameters left
    iSOAP('ws.tof.oa.com', '/MessageService.svc', 'http://tempuri.org/IMessageService/SendWeiXinMessage', data, function(res) {
        cb(res);
    });
    data = null;
};
...
exports.signin = function(user, password, cb) {
    ...
    iSOAP('passport.oa.com', '/services/passportservice.asmx', 'http://indigo.oa.com/services/AuthenticateViaToken', data, function(res) {
        cb(res);
    });
    data = null;
};